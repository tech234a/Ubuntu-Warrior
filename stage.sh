...
rm /etc/inittab
cat <<EOT >> /etc/inittab
::sysinit:/bin/sh -c "if [ -f /root/splashes/at-splash-startup-640x400-32.fb ]; then /bin/echo -n -e '\e[?17;14;244c'; cat /root/splashes/at-splash-startup-640x400-32.fb > /dev/fb0; /sbin/openrc sysinit --quiet > /dev/null 2>&1; /sbin/openrc boot --quiet > /dev/null 2>&1; else /sbin/openrc sysinit; /sbin/openrc boot; fi"
