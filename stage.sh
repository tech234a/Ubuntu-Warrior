...
# /etc/inittab
# On startup, check for the startup splash. If it exists, run disable cursor blinking and run
# OpenRC silently (including errors). Otherwise, boot normally.
rm /etc/inittab
cat <<EOT >> /etc/inittab
::sysinit:/bin/sh -c "if [ -f /root/splashes/at-splash-startup-640x400-32.fb ]; then /bin/echo -n -e '\e]P0cccccc'; /bin/echo -n -e '\e[?17;14;100c'; cat /root/splashes/at-splash-startup-640x400-32.fb > /dev/fb0; /sbin/openrc sysinit --quiet > /dev/null 2>&1; /sbin/openrc boot --quiet > /dev/null 2>&1; else /sbin/openrc sysinit; /sbin/openrc boot; fi"
